...
ES6 modules loaded via `<script type="module" src="main.js">`. Non-module `colors.js` loads in `<head>` to freeze `_PALETTE` before modules run. Loads `/shared-tokens.js`, `/shared-utils.js`, `/shared-camera.js`, `/shared-base.css`, `/shared-info.js`, and `/shared-shortcuts.js` from the root site.

```
main.js                 — Entry point: imports, DOM cache ($), init, setupUI(), shortcuts, info tips
src/
  config.js             — CONFIG, hex geometry constants, EASE_OUT, getHexesInRadius()
  ...
  state.js              — state object, initDistricts, undo/redo, snapshot, brushSize
  metrics.js            — compactness, contiguity, efficiency gap, partisan symmetry, competitive districts, required MMD
  renderer.js           — renderMap, renderBorders, renderDistrictLabels, hex visuals
  input.js              — mouse handlers, painting logic, hover/tooltip, paintBrush, autoFillDistrict
  touch.js              — pinch-zoom, pan, touch-paint
  zoom.js               — camera init (bindZoomButtons), zoomToFit, shiftForSidebar
  sidebar.js            — updateMetrics UI, proportionality display, new metric rendering
...
- **Efficiency gap**: All-party wasted-vote analysis using plurality thresholds. Winner's wasted votes = votes above second-place + 1; each loser's wasted = all their votes. Returns `{ red, blue, yellow }` wasted-vote ratios. Sidebar shows dominant advantage.
- **Partisan symmetry**: For each party pair, swap vote shares across all districts and recount seats. Deviation = |actual seat gap − hypothetical seat gap|. Returns 0–100% (100 = perfectly symmetric).
- **Competitive districts**: Count districts where margin of victory < 10%.
- **Required MMD**: Dynamic majority-minority district requirement derived from `floor(minorityShare × numDistricts)` (min 1 if minority > 15%).
...
- `loadPlan(name)`: restores hex assignments, recalculates metrics and visuals
- `deletePlan(name)`: removes from localStorage
- `exportPlan(name)` / `exportCurrentPlan(name)`: triggers JSON file download
- `importPlan(file)`: reads JSON, validates structure, saves to localStorage
- `listPlans()`: returns array of `{ name, timestamp }`
- Plans dialog reuses toolbar glass panel pattern, opened via Plans button in toolbar

### Brush & Auto-Fill (src/input.js)

- **Brush sizes**: 1 (single hex), 3 (center + 6 neighbors), 7 (center + 18 neighbors). Toggle buttons in toolbar set `state.brushSize` (0/1/2 = radii).
- `paintBrush(qr)`: wraps `paintHexByKey()` with `getHexesInRadius()` from config.js. Delegates to single-hex paint per neighbor, preserving population cap enforcement.
- `autoFillDistrict(districtId)`: greedy nearest-neighbor algorithm. Finds unassigned hexes adjacent to district boundary, sorts by distance to centroid, adds greedily until `district.population ≥ targetPop × 1.1`. Pushes single undo snapshot, shows toast with count.

### User Interactions

- Left-click: assign hex to current district (with 110% population cap); brush size affects radius
...
- Plans button: open save/load/export/import dialog
- Auto-fill button: greedy-fill current district to population cap

### Keyboard Shortcuts & Info Tips

- **Shortcuts** via `initShortcuts()` from `shared-shortcuts.js`: E (erase mode), D (delete mode), A (auto-fill), N (randomize map), 0-9 (select district), T (theme), S (sidebar).
- **Info tips** via `createInfoTip()` from `shared-info.js`: `?` buttons next to each metric — Efficiency Gap, Partisan Symmetry, Competitive Districts, Compactness, Contiguity, Majority-Minority, Population Balance. Data defined inline in `main.js`.
